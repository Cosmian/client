...
use cosmian_kms_rest_client::{batch_export_objects, ExportObjectParams, KmsRestClient};
...
pub(crate) fn get_kms_client() -> Result<KmsRestClient, Pkcs11Error> {
    let conf_path = ClientConf::location(None)?;
    let conf = ClientConf::load(&conf_path)?;
    let kms_client = KmsRestClient::new(conf)?;
    Ok(kms_client)
}
