...
use cosmian_kms_rest_client::{import_object, KmsRestClient};
...
async fn test_kms_client() -> Result<(), Pkcs11Error> {
    ...
    let kms_client = KmsRestClient::new(ctx.owner_client_conf.clone())?;
...
fn initialize_backend() -> Result<CkmsBackend, Pkcs11Error> {
    cosmian_logger::log_utils::log_init(Some("fatal,cosmian_kms_rest_client=debug"));
    ...
    Ok(CkmsBackend::instantiate(KmsRestClient::new(
        owner_client_conf.clone(),
    )?))
}

async fn create_keys(kms_client: &KmsRestClient) -> Result<(), Pkcs11Error> {
