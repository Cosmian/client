...
pub(crate) fn policy_from_binary_file(bin_filename: &impl AsRef<Path>) -> CosmianResult<Policy> {
...
pub(crate) fn policy_from_json_file(specs_filename: &impl AsRef<Path>) -> CosmianResult<Policy> {
...
) -> CosmianResult<Policy> {
    // Recover the KMIP Object
    let object: Object = if let Some(key_id) = key_id {
        export_object(kms_rest_client, key_id, ExportObjectParams {
            unwrap,
