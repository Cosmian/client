...
impl LoginAction {
    ...
    pub async fn process(&self, conf_path: &PathBuf) -> CliResult<()> {
        ...
        let findex_client_conf = conf.clone().findex_client_conf.ok_or_else(|| {
            CliError::Default(format!(
                "The `login` command used for Findex REST client cannot be empty"
            ))
        })?;
        let oauth2_conf = findex_client_conf.oauth2_conf.as_ref().ok_or_else(|| {
            CliError::Default(format!(
