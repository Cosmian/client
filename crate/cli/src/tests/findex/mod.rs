...
use cosmian_findex_rest_client::Permission;
...
use crate::{
    actions::findex::{
        add_or_delete::AddOrDeleteAction,
        parameters::FindexParameters,
        permissions::{GrantPermission, RevokePermission},
        search::SearchAction,
    },
    error::result::CliResult,
    tests::findex::permissions::{
        create_index_id_cmd, grant_permission_cmd, revoke_permission_cmd,
    },
};
...
pub(crate) async fn test_findex_no_auth() -> CliResult<()> {
    ...
    let ctx_findex = start_default_test_findex_server().await;
    add_search_delete(&ctx_findex.owner_client_conf_path, &Uuid::new_v4())?;
    Ok(())
}
