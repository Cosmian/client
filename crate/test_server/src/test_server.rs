...
use cosmian_config::{ClientConf, GmailApiConf, HttpClientConf};
use cosmian_kms_rest_client::{
    cosmian_kmip::crypto::{secret::Secret, symmetric::symmetric_ciphers::AES_256_GCM_KEY_LENGTH},
    kms_client_bail, kms_client_error, write_json_object_to_file, KmsRestClient,
    KmsRestClientError,
};
...
use crate::test_jwt::{get_kms_auth0_jwt_config, KMS_AUTH0_TOKEN};
...
pub struct TestsContext {
    ...
    pub thread_handle: JoinHandle<Result<(), KmsRestClientError>>,
}

impl TestsContext {
    pub async fn stop_server(self) -> Result<(), KmsRestClientError> {
...
pub async fn start_test_server_with_options(
...
) -> Result<TestsContext, KmsRestClientError> {
    ...
    let kms_client = KmsRestClient::new(owner_client_conf.clone())?;

    info!(
        "Starting KMS test server at URL: {} with server params {:?}",
        owner_client_conf.kms_client_conf.server_url, &server_params
    );
    ...
    if db_config.database_type.clone().unwrap() == "sqlite-enc" {
        ...
        owner_client_conf.kms_client_conf.kms_database_secret = Some(database_secret);
...
fn start_test_kms_server(
...
) -> (ServerHandle, JoinHandle<Result<(), KmsRestClientError>>) {
    ...
    let thread_handle = thread::spawn(move || {
        ...
        tokio::runtime::Builder::new_multi_thread()
            ...
            .map_err(|e| KmsRestClientError::UnexpectedError(e.to_string()))
    });
...
async fn wait_for_server_to_start(kms_client: &KmsRestClient) -> Result<(), KmsRestClientError> {
...
fn generate_server_params(
    ...
    ServerParams::try_from(clap_config).map_err(|e| {
        KmsRestClientError::Default(format!("failed initializing the server config: {e}"))
    })
}
...
fn set_access_token(server_params: &ServerParams, api_token: Option<String>) -> Option<String> {
    if server_params.identity_provider_configurations.is_some() {
        trace!("Setting access token for JWT: {KMS_AUTH0_TOKEN:?}");
        Some(KMS_AUTH0_TOKEN.to_string())
...
) -> Result<(String, ClientConf), KmsRestClientError> {
    // This create root dir
    let root_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        ...
        kms_client_conf: HttpClientConf {
            ...
            ssl_client_pkcs12_path: if server_params.authority_cert_file.is_some() {
                ...
                Some(
                    p.to_str()
                        .ok_or_else(|| {
                            KmsRestClientError::Default("Can't convert path to string".to_owned())
                        })?
                        .to_string(),
                )
            } else {
...
fn generate_user_conf(
...
) -> Result<String, KmsRestClientError> {
    // This create root dir
    let root_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));

    let mut user_conf = owner_client_conf.clone();
    user_conf.kms_client_conf.ssl_client_pkcs12_path = {
        #[cfg(not(target_os = "macos"))]
        let p = root_dir
    ...
    user_conf.kms_client_conf.ssl_client_pkcs12_password = Some("password".to_owned());
...
pub fn generate_invalid_conf(correct_conf: &ClientConf) -> String {
    ...
    invalid_conf.kms_client_conf.kms_database_secret = Some(token);
...
#[cfg(test)]
#[tokio::test]
async fn test_start_server() -> Result<(), KmsRestClientError> {
