...
use cosmian_findex_rest_client::{
    findex_client_bail, findex_client_error, FindexRestClient, FindexRestClientError,
};
...
use cosmian_kms_rest_client::write_json_object_to_file;
use cosmian_logger::log_utils::log_init;
use tokio::sync::OnceCell;
use tracing::{info, trace};

use crate::test_jwt::{get_findex_auth0_jwt_config, FINDEX_AUTH0_TOKEN};
...
pub struct TestsContext {
    ...
    pub thread_handle: JoinHandle<Result<(), FindexRestClientError>>,
}

impl TestsContext {
    pub async fn stop_server(self) -> Result<(), FindexRestClientError> {
...
) -> Result<TestsContext, FindexRestClientError> {
    log_init(None);
...
fn start_test_findex_server(
...
) -> (ServerHandle, JoinHandle<Result<(), FindexRestClientError>>) {
    ...
    let thread_handle = thread::spawn(move || {
        ...
        tokio::runtime::Builder::new_multi_thread()
            ...
            .map_err(|e| FindexRestClientError::UnexpectedError(e.to_string()))
    });
...
fn generate_server_params(
    ...
    ServerParams::try_from(clap_config).map_err(|e| {
        FindexRestClientError::Default(format!("failed initializing the server config: {e}"))
    })
}

fn set_access_token(server_params: &ServerParams) -> Option<String> {
    if server_params.identity_provider_configurations.is_some() {
        trace!("Setting access token for JWT: {FINDEX_AUTH0_TOKEN:?}");
        Some(FINDEX_AUTH0_TOKEN.to_string())
    } else {
        None
    }
}
...
fn generate_owner_conf(
    server_params: &ServerParams,
) -> Result<(String, ClientConf), FindexRestClientError> {
    // This create root dir
    let root_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    ...
    let owner_client_conf = ClientConf {
        kms_client_conf: HttpClientConf {
            ...
            ssl_client_pkcs12_path: if server_params.authority_cert_file.is_some() {
                ...
                Some(
                    p.to_str()
                        .ok_or_else(|| {
                            FindexRestClientError::Default(
                                "Can't convert path to string".to_owned(),
                            )
                        })?
                        .to_string(),
                )
        ...
        findex_client_conf: Some(HttpClientConf {
            server_url: if matches!(server_params.http_params, HttpParams::Https(_)) {
                format!("https://0.0.0.0:{}", server_params.port)
            } else {
                format!("http://0.0.0.0:{}", server_params.port)
...
fn generate_user_conf(
    port: u16,
    owner_client_conf: &ClientConf,
) -> Result<String, FindexRestClientError> {
    // This create root dir
    let root_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));

    let ssl_client_pkcs12_path = {
        #[cfg(not(target_os = "macos"))]
        let p = root_dir
        ...
        Some(
            p.to_str()
                .ok_or_else(|| {
                    FindexRestClientError::Default("Can't convert path to string".to_owned())
                })?
                .to_string(),
        )
    };
    let mut user_conf = owner_client_conf.clone();
    user_conf.findex_client_conf = Some(HttpClientConf {
        accept_invalid_certs: true,
        server_url: if owner_client_conf
            .clone()
            .findex_client_conf
            .unwrap()
            .server_url
        ...
        ssl_client_pkcs12_path: ssl_client_pkcs12_path.clone(),
    ...
    write_json_object_to_file(&user_conf, &user_conf_path)
        .map_err(|e| findex_client_error!("Can't write user CLI conf path: {e}"))?;
...
mod test {
    ...
    use crate::test_server_findex::{
        redis_db_config, start_test_server_with_options, AuthenticationOptions,
    };

    #[tokio::test]
    async fn test_server_auth_matrix() -> Result<(), FindexRestClientError> {
        let test_cases = vec![
