...
use cosmian_kms_rest_client::{KmsRestClient, KmsRestClientError};
...
pub(crate) fn bench_create_symmetric_key(c: &mut Criterion) {
    ...
    let kms_rest_client = runtime.block_on(async {
        ...
        KmsRestClient::new(ctx.owner_client_conf.clone()).unwrap()
    });
...
) -> Result<UniqueIdentifier, KmsRestClientError> {
...
fn create_symmetric_key_request<T: IntoIterator<Item = impl AsRef<str>>>(
    ...
    cryptographic_parameters: CryptographicParameters,
    tags: T,
) -> Result<Create, KmsRestClientError> {
...
pub(crate) fn bench_encrypt(
    ...
    let (kms_rest_client, key_id) = runtime.block_on(async {
        let ctx = start_default_test_kms_server().await;
        let kms_client = KmsRestClient::new(ctx.owner_client_conf.clone()).unwrap();
...
pub(crate) async fn encrypt(
...
) -> Result<(Option<Vec<u8>>, Vec<u8>, Option<Vec<u8>>), KmsRestClientError> {
    ...
    let data = encrypt_response
        .data
        .ok_or_else(|| KmsRestClientError::UnexpectedError("No data".to_string()))?;
...
) -> Result<Encrypt, KmsRestClientError> {
    Ok(Encrypt {
...
pub(crate) async fn decrypt(
    ...
    ciphertext: Vec<u8>,
    mac: Option<Vec<u8>>,
) -> Result<Zeroizing<Vec<u8>>, KmsRestClientError> {
    // Create the kmip query
    let decrypt_request =
...
fn decrypt_request(
    ...
    data: Vec<u8>,
    mac: Option<Vec<u8>>,
) -> Result<Decrypt, KmsRestClientError> {
    Ok(Decrypt {
        unique_identifier: Some(key_id),
        cryptographic_parameters: Some(cryptographic_parameters),
        data: Some(data),
