...
use crate::{
    ...
    error::result::{KmsRestClientResult, KmsRestClientResultHelper},
    KmsRestClient, KmsRestClientError,
};
...
) -> Result<(Object, Option<Attributes>), KmsRestClientError> {
...
pub async fn batch_export_objects(
    ...
    params: ExportObjectParams<'_>,
) -> Result<Vec<(Object, Attributes)>, KmsRestClientError> {
...
async fn batch_get(
    ...
    block_cipher_mode: Option<BlockCipherMode>,
    authenticated_encryption_additional_data: Option<String>,
) -> KmsRestClientResult<Vec<(Object, Attributes)>> {
    ...
    for response in responses.chunks(2) {
        match response {
            ...
            operations => {
                ...
                return Err(KmsRestClientError::Default(format!(
