...
use crate::error::{result::KmsRestClientResultHelper, KmsRestClientError};
...
impl KmsRestClient {
    ...
    pub fn new(conf: ClientConf) -> Result<KmsRestClient, KmsRestClientError> {
        // Instantiate a KMS server REST client with the given configuration
        let kms_rest_client = HttpClient::instantiate(
            &conf.kms_client_conf.server_url,
            conf.kms_client_conf.access_token.as_deref(),
            conf.kms_client_conf.ssl_client_pkcs12_path.as_deref(),
            conf.kms_client_conf.ssl_client_pkcs12_password.as_deref(),
            conf.kms_client_conf.kms_database_secret.as_deref(),
            conf.kms_client_conf.accept_invalid_certs,
            if let Some(certificate) = &conf.kms_client_conf.verified_cert {
                Some(Certificate(
    ...
    pub async fn certify(&self, request: Certify) -> Result<CertifyResponse, KmsRestClientError> {
    ...
    pub async fn create(&self, request: Create) -> Result<CreateResponse, KmsRestClientError> {
    ...
    ) -> Result<CreateKeyPairResponse, KmsRestClientError> {
    ...
    pub async fn decrypt(&self, request: Decrypt) -> Result<DecryptResponse, KmsRestClientError> {
    ...
    pub async fn destroy(&self, request: Destroy) -> Result<DestroyResponse, KmsRestClientError> {
    ...
    /// The success or failure of the operation is indicated by the Result
    /// Status (and if failure the Result Reason) in the response header.
    pub async fn encrypt(&self, request: Encrypt) -> Result<EncryptResponse, KmsRestClientError> {
    ...
    pub async fn export(&self, request: Export) -> Result<ExportResponse, KmsRestClientError> {
    ...
    pub async fn get(&self, request: Get) -> Result<GetResponse, KmsRestClientError> {
    ...
    ) -> Result<GetAttributesResponse, KmsRestClientError> {
    ...
    ) -> Result<SetAttributeResponse, KmsRestClientError> {
    ...
    ) -> Result<DeleteAttributeResponse, KmsRestClientError> {
    ...
    pub async fn import(&self, request: Import) -> Result<ImportResponse, KmsRestClientError> {
    ...
    /// indicator.
    pub async fn locate(&self, request: Locate) -> Result<LocateResponse, KmsRestClientError> {
    ...
    ) -> Result<ReKeyKeyPairResponse, KmsRestClientError> {
    ...
    pub async fn rekey(&self, request: ReKey) -> Result<ReKeyResponse, KmsRestClientError> {
    ...
    pub async fn revoke(&self, request: Revoke) -> Result<RevokeResponse, KmsRestClientError> {
    ...
    pub async fn validate(
        &self,
        request: Validate,
    ) -> Result<ValidateResponse, KmsRestClientError> {
    ...
    pub async fn message(&self, request: Message) -> Result<MessageResponse, KmsRestClientError> {
    ...
    pub async fn new_database(&self) -> Result<String, KmsRestClientError> {
    ...
    pub async fn grant_access(
        &self,
        access: Access,
    ) -> Result<SuccessResponse, KmsRestClientError> {
    ...
    pub async fn list_access(
        &self,
        uid: &str,
    ) -> Result<Vec<UserAccessResponse>, KmsRestClientError> {
    ...
    pub async fn list_owned_objects(&self) -> Result<Vec<ObjectOwnedResponse>, KmsRestClientError> {
    ...
    pub async fn list_access_rights_obtained(
        &self,
    ) -> Result<Vec<AccessRightsObtainedResponse>, KmsRestClientError> {
        self.get_no_ttlv("/access/obtained", None::<&()>).await
    }

    /// This operation requests the version of the server
    pub async fn version(&self) -> Result<String, KmsRestClientError> {
        self.get_no_ttlv("/version", None::<&()>).await
    }

    /// This operation requests `google_cse` status of the server
    pub async fn google_cse_status(&self) -> Result<StatusResponse, KmsRestClientError> {
        ...
        O: Serialize,
    {
        ...
        Err(KmsRestClientError::RequestFailed(p))
    }

    pub async fn delete_no_ttlv<O, R>(
        &self,
        endpoint: &str,
        data: &O,
    ) -> Result<R, KmsRestClientError>
    ...
    pub async fn post_ttlv<O, R>(&self, kmip_request: &O) -> Result<R, KmsRestClientError>
    where
        O: Serialize,
        R: serde::de::DeserializeOwned + Sized + 'static,
    {
        ...
        if status_code.is_success() {
            ...
            return from_ttlv(&ttlv).map_err(|e| KmsRestClientError::ResponseFailed(e.to_string()))
        }
...
async fn handle_error(endpoint: &str, response: Response) -> Result<String, KmsRestClientError> {
