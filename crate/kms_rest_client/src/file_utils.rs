...
use crate::{
    ...
    error::{result::KmsRestClientResultHelper, KmsRestClientError},
};

/// Read all bytes from a file
pub fn read_bytes_from_file(file: &impl AsRef<Path>) -> Result<Vec<u8>, KmsRestClientError> {
...
pub fn read_from_json_file<T>(file: &impl AsRef<Path>) -> Result<T, KmsRestClientError>
where
    T: DeserializeOwned,
{
...
pub fn read_object_from_json_ttlv_bytes(bytes: &[u8]) -> Result<Object, KmsRestClientError> {
...
pub fn read_object_from_json_ttlv_file(
    object_file: &PathBuf,
) -> Result<Object, KmsRestClientError> {
...
pub fn read_bytes_from_files_to_bulk(
    input_files: &[PathBuf],
) -> Result<Vec<u8>, KmsRestClientError> {
    let mut ser = Serializer::new();

    // number of files to decrypt
    let nb_input_files = u64::try_from(input_files.len()).map_err(|_| {
        KmsRestClientError::Conversion(format!(
