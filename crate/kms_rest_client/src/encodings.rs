...
use crate::{kms_client_bail, KmsRestClientError};
...
/// | PKCS12 | PKCS#12 |
///
pub fn objects_from_pem(bytes: &[u8]) -> Result<Vec<Object>, KmsRestClientError> {
    ...
    for pem in pem_s {
        ...
        match pem.tag() {
            ...
            "EC PUBLIC KEY" => {
                return Err(KmsRestClientError::NotSupported(
            ...
            x => {
                return Err(KmsRestClientError::NotSupported(format!(
