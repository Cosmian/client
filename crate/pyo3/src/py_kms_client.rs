...
use cosmian_kms_rest_client::KmsRestClient as RustKmsClient;
use pyo3::{
...
impl KmsClient {
    ...
    pub fn new(
        ...
        let kms_connector = RustKmsClient::new(ClientConf {
            kms_client_conf: HttpClientConf {
                accept_invalid_certs: insecure_mode,
                server_url: server_url.to_owned(),
                verified_cert: allowed_tee_tls_cert.map(|cert| cert.to_owned()),
                access_token: api_key.map(|secret| secret.to_owned()),
                ssl_client_pkcs12_path: client_pkcs12_path.map(|path| path.to_owned()),
                ssl_client_pkcs12_password: client_pkcs12_password
                    .map(|password| password.to_owned()),
                kms_database_secret: database_secret.map(|secret| secret.to_owned()),
